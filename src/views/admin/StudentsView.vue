  ...
  reader.onload = (e) => {
    ...
    outer: for (const row of rows) {
      for (const cell of row) {
        ...
        if (m) { detectedClassName.value = m[1].trim(); break outer }
      }
    }
    ...
    if (detectedClassName.value) {
      const match = classes.value.find(
        c => c.class_name.trim().toLowerCase() === detectedClassName.value.toLowerCase()
      )
    ...
    const dataRows = rows.filter(row => /^\d+$/.test(String(row[0] || '').trim()))
    ...
    importRows.value = dataRows.map(row => {
      ...
      const fatherName = String(row[5] || '').trim().replace(/^\s+$/, '')
      const motherName = String(row[6] || '').trim().replace(/^\s+$/, '')
      const address    = String(row[7] || '').trim()
      // row[8] = ethnicity — not in schema, skip
      const disability = String(row[9] || '').trim()

      const gender = genderRaw === 'ស្រី' ? 'Female' : genderRaw === 'ប្រុស' ? 'Male' : ''
    ...
    <div class="card">
      <div v-if="loading" class="card-body">
        <div v-for="i in 6" :key="i" class="skeleton"
             style="height:52px;margin-bottom:10px;border-radius:8px;"></div>
      </div>
      ...
      <div v-else class="table-wrapper">
        <table>
          <thead>
            <tr>
              <th>សិស្ស</th><th>អត្តលេខ</th><th>ភេទ</th>
              <th>ថ្ងៃខែឆ្នាំកំណើត</th><th>ថ្នាក់</th><th>ស្ថានភាព</th><th>សកម្មភាព</th>
            </tr>
          </thead>
          <tbody>
            <tr v-for="s in filtered" :key="s.id"
                style="cursor:pointer;" @click="router.push('/admin/students/'+s.id)">
              <td @click.stop>
              ...
              <td>
                <span class="badge" :class="s.gender === 'Male' ? 'badge-blue' : 'badge-red'">
                  {{ s.gender === 'Male' ? 'ប្រុស' : s.gender === 'Female' ? 'ស្រី' : '—' }}
                </span>
              </td>
              ...
              <td @click.stop>
                <div class="table-actions">
                  <button class="btn btn-ghost btn-sm btn-icon"
                          @click="router.push('/admin/students/'+s.id)" title="លម្អិត">
                    <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2">
                  ...
                  <button class="btn btn-danger btn-sm btn-icon"
                          @click="deleteTarget = s" title="លុប">
                    <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2">
      ...
      <div class="modal modal-lg">
        ...
        <div class="modal-body">
          <div style="display:grid;grid-template-columns:1fr 1fr;gap:14px;">
            ...
            <div class="form-group" style="grid-column:1/-1;">
              ...
              <textarea class="form-textarea" v-model="form.address" rows="2"
                        placeholder="អាសយដ្ឋានផ្ទះ"></textarea>
            </div>
            <div class="form-group">
            ...
            <div class="form-group" style="grid-column:1/-1;display:flex;gap:24px;">
              <label style="display:flex;align-items:center;gap:8px;cursor:pointer;font-size:13px;">
                <input type="checkbox" v-model="form.is_scholarship"
                       style="width:15px;height:15px;" />
                ទទួលបានអាហារូបករណ៍
              </label>
              <label style="display:flex;align-items:center;gap:8px;cursor:pointer;font-size:13px;">
                <input type="checkbox" v-model="form.is_disability"
                       style="width:15px;height:15px;" />
                មានពិការភាព
              </label>
            </div>
        ...
        <div class="modal-footer">
          ...
          <button class="btn btn-primary" @click="save" :disabled="saving">
            {{ saving ? 'កំពុងរក្សាទុក...' : isEdit ? 'រក្សាទុកការកែប្រែ' : 'បន្ថែមសិស្ស' }}
          </button>
        </div>
      </div>
    </div>
    ...
    <div v-if="showImportModal" class="modal-overlay" @click.self="showImportModal = false">
      <div class="modal"
        ...
        <div class="modal-header">
          ...
          <button class="btn btn-ghost btn-sm btn-icon" @click="showImportModal = false">
            <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2">
              <line x1="18" y1="6" x2="6" y2="18"/>
              <line x1="6" y1="6" x2="18" y2="18"/>
            </svg>
        ...
        <div class="modal-body"
             style="overflow-y:auto;flex:1;display:flex;flex-direction:column;gap:16px;">

          <!-- ── Drop zone (no file yet) ──────────────────────────────── -->
          <div v-if="importRows.length === 0"
            ...
            <p style="font-size:12px;color:var(--text-muted);">.xlsx · .xls</p>
          ...
          <template v-else>
            ...
            <div style="display:flex;align-items:center;gap:10px;flex-wrap:wrap;">
              ...
              <span v-if="skippedRows.length"
                    class="badge badge-yellow" style="font-size:12px;padding:4px 10px;">
            ...
            <div class="form-group" style="margin:0;">
              ...
              <div v-if="detectedClassName && !importClass"
                ...
                <button class="btn btn-sm btn-primary"
                  ...
                  {{ creatingClass ? 'កំពុងបង្កើត...' : `+ បង្កើតថ្នាក់ "${detectedClassName}"` }}
                </button>
              </div>

              <!-- Detected and matched → success hint -->
              <div v-else-if="detectedClassName && importClass"
                   class="import-class-banner import-class-banner--success">
                <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"
                     style="width:15px;height:15px;flex-shrink:0;">
                  <polyline points="20 6 9 17 4 12"/>
                </svg>
                បានរកឃើញថ្នាក់ <strong>{{ detectedClassName }}</strong> ដោយស្វ័យប្រវត្តិ
              </div>

              <!-- Always-visible dropdown so user can override -->
              <select class="form-select" v-model="importClass"
            ...
            <div style="overflow-x:auto;border:1px solid var(--border);border-radius:8px;">
              <table style="min-width:620px;">
                ...
                <tbody>
                  <tr v-for="r in importRows" :key="r._num"
                      :style="!r.valid
                        ? 'opacity:.4;background:var(--bg-secondary,#f9fafb);'
                        : ''">
                    <td style="font-size:12px;color:var(--text-muted);">{{ r._num }}</td>
                    <td style="font-weight:600;font-size:13px;">{{ r.fullName || '—' }}</td>
                    <td>
                      <span v-if="r.gender" class="badge"
                            :class="r.gender === 'Male' ? 'badge-blue' : 'badge-red'">
                    ...
                    <td style="text-align:center;">
                      <span v-if="r.isDisability"
                            class="badge badge-yellow" style="font-size:11px;">បាទ</span>
                      <span v-else style="color:var(--text-muted);font-size:12px;">—</span>
                    </td>
                    <td>
                      <span v-if="r.valid"
                            class="badge badge-green" style="font-size:11px;">OK</span>
                      <span v-else class="badge badge-red" style="font-size:11px;"
                            :title="r.reasons.join(', ')">រំលង</span>
                    </td>
                  </tr>
                </tbody>
              </table>
            ...
            <p v-if="skippedRows.length"
               style="font-size:12px;color:var(--text-muted);margin:0;">
              ⚠ ជួររំលង = គ្មានឈ្មោះ ឬថ្ងៃខែឆ្នាំកំណើតមិនត្រឹមត្រូវ (ត្រូវការទម្រង់ DD/MM/YYYY)។
            </p>
        ...
        <div class="modal-footer">
          <button class="btn btn-ghost" @click="showImportModal = false">បោះបង់</button>
          <button v-if="importRows.length > 0"
                  class="btn btn-primary"
                  :disabled="importing || validRows.length === 0"
                  @click="doImport">
            {{ importing ? 'កំពុងបន្ថែម...' : `បន្ថែមសិស្ស ${validRows.length} នាក់` }}
          </button>
        </div>
