# 🏫 Primary School Management System — Roadmap v12
...
- [ ] Run `migrate_v6_to_v7.sql` on Supabase project
...
### 🔧 Phase 7 — Student Rollup (v7) ← CURRENT

- [ ] Run `migrate_v6_to_v7.sql` on Supabase
- [ ] Build rollup modal on `/admin/academic-years`:
  - Year selector (source → target)
  - Preview mapping table per grade
  - Confirm button → calls `perform_student_rollup()`
  - Result summary (X promoted, X graduated, warnings for skipped)
- [ ] Test with real data at Battambang school
- [ ] Handle edge case: students already rolled up (re-run protection)

---

### 🔧 Phase 8 — Stabilize & Polish
